											...
											<TBODY>
												...
												<c:forEach items="${ page.beanList }" var="linkman">
													<TR
														...
														<TD><a
															href="${pageContext.request.contextPath }/linkman_editInit.action?lkm_id=${linkman.lkm_id}">修改</a>
															&nbsp;&nbsp; <a
															href="${pageContext.request.contextPath }/linkman_delete?lkm_id=${linkman.lkm_id}" onclick="return window.confirm('确定要删除吗？删除后将无法回复!')">删除</a>
														</TD>
													</TR>
												</c:forEach>
									...
									<TD><SPAN id=pagelink>
											<DIV
												...
												,每页显示 <select name="pageSize">
													<c:forEach begin="1" end="${page.totalCount}"
														varStatus="vs">
														<option value="${vs.count}"
															<c:if test="${page.pageSize==vs.count }">selected</c:if>>${vs.count}</option>
													</c:forEach>
												</select> 条
