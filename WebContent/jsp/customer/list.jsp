										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR
													...
													<TD><A href="${pageContext.request.contextPath}/customer_downLoad.action?cust_id=${customer.cust_id}">下载文件</A></TD>
													<TD>
													<a href="${pageContext.request.contextPath }/customer_initUpdate.action?cust_id=${customer.cust_id}">修改</a>
													&nbsp;&nbsp;
													<a href="${pageContext.request.contextPath }/customer_delete?cust_id=${customer.cust_id}" onclick="return window.confirm('确定要删除吗？删除后将无法恢复!')">删除</a>
													</TD>
												</TR>
												</c:forEach>
										...
										<SPAN id=pagelink>
											<DIV style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right">
												...
												<select name="pageSize">
													<c:forEach begin="1" end="${page.totalCount}" varStatus="vs">
														<option value="${vs.count}" <c:if test="${page.pageSize==vs.count }">selected</c:if>>${vs.count}</option>
													</c:forEach>
												</select>
												条
